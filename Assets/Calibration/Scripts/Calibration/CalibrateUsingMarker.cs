...
public abstract class Trackeable: Component{
    public abstract Vector3 GetPosition();
    public abstract Quaternion GetOrientation();
    public abstract bool IsTracked();
}

public class CalibrateUsingMarker : MonoBehaviour
{
    Camera _camera;
    ...
    [SerializeField] Vector2[] targetUV = new Vector2[0]; //fixed UV points to sample

    [Header("Target properties (REQUIRED)")]
    [SerializeField] float near = 0.1f;
    [SerializeField] float far = 100;
    [SerializeField] Trackeable target;
    ...
    [Header("Samples")]
    [SerializeField] List<Vector2> uv = new List<Vector2>(); //sampled uv
    [SerializeField] List<Vector3> xyz = new List<Vector3>(); //sampled xyz
